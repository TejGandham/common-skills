...
<critical>YOU MUST ALWAYS SPEAK OUTPUT In your Agent communication style with the configured `English`</critical>
<critical>YOU MUST ALWAYS WRITE all artifact and document content in `English`</critical>
...
<action>Check if docs/index.md exists</action>
...
<action>Initialize state file: docs/project-scan-report.json</action>
...
<action>IMMEDIATELY write to: docs/api-contracts-{part_id}.md</action>
...
<action>IMMEDIATELY write to: docs/data-models-{part_id}.md</action>
...
<action>Read docs/index.md</action>
...
3. For UI-only features: Reference docs/architecture-{{ui_part_id}}.md
4. For API-only features: Reference docs/architecture-{{api_part_id}}.md
...
<action>Display: "State file saved: docs/project-scan-report.json"</action>
