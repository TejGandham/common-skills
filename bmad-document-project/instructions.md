...
<action>Check for existing state file at: docs/project-scan-report.json</action>
...
<ask>I found an in-progress workflow state from {{last_updated}}.
  ...
  <check if="user selects 2">
    <action>Create archive directory: docs/.archive/</action>
    <action>Move old state file to: docs/.archive/project-scan-report-{{timestamp}}.json</action>
  ...
  <check if="state file age >= 24 hours">
    ...
    <action>Archive old state file to: docs/.archive/project-scan-report-{{timestamp}}.json</action>
    <action>Set resume_mode = false</action>
    <action>Continue to Step 0.5</action>
  </check>
