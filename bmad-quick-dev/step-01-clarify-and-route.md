---
wipFile: 'docs/implementation/spec-wip.md'
deferred_work_file: 'docs/implementation/deferred-work.md'
...
3. Otherwise — scan artifacts and ask
   ...
   - Active specs (`ready-for-dev`, `in-progress`, `in-review`) in `docs/implementation`? → List them and HALT. Ask user which to resume (or `[N]` for new).
...
1. Load context.
   - List files in `docs` and `docs/implementation`.
...
5. Route — choose exactly one:
   ...
   **b) Plan-code-review** — everything else. When uncertain whether blast radius is truly zero, choose this path.
   1. Derive a valid kebab-case slug from the clarified intent. If the intent references a tracking identifier (story number, issue number, ticket ID), lead the slug with it (e.g. `3-2-digest-delivery`, `gh-47-fix-auth`). If `docs/implementation/spec-{slug}.md` already exists, append `-2`, `-3`, etc. Set `spec_file` = `docs/implementation/spec-{slug}.md`.
