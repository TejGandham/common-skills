...
- Also ban by *pattern*, not just token: default-shipped fonts of whatever framework the project uses, accent hues in the 240–280° range paired with a second gradient stop, three-column feature grids with generic line icons, "trusted by" logo walls with no typographic point of view
- The point is "pick a genre with opinions," not "pick loud." Editorial restraint, Swiss precision, and industrial minimalism are anti-slop. Brutalism and maximalism are anti-slop. Tasteful-neutral-but-indistinguishable-from-every-other-YC-SaaS is slop.
- Vary themes, fonts, aesthetics
- Match complexity to vision. For trust-sensitive categories (compliance, fintech, healthcare), "distinctive restraint" is often the right axis. For crowded-generic categories (dev tools, consumer), lean further toward unexpected.
...
### 2.2 Design Proposal
- If the brief leaves real room for interpretation (tone, visual language, information density), propose 2-3 approaches with trade-offs so the user can choose. If the brief already pins the vision, skip to 2.3 and commit — forcing alternatives on a clear brief is busywork, and the "commit to vision" rule in Design Thinking wins.
...
- Name the **ONE memorable thing** explicitly inside Visual Theme — the signature detail other products don't have
...
- On create: omit `changed_tokens` or return `[]`. On update: include `changed_tokens: [token_name, ...]` listing only the tokens that changed.
...
- Check color contrast (4.5:1 text, 3:1 large) — compute the ratio, don't eyeball it
- Verify ARIA labels/roles present
- Check focus indicators
- Verify semantic HTML
- Check touch targets (min 44x44px)
- **Interactive-element semantics:** flag `<div onClick>`, `<span onClick>`, and other non-semantic elements handling activation. Require `<button>` / `<a>` / `<input type="submit">` — this is one of the most common React a11y bugs and a critical finding.
- **Form-specific a11y** (when target contains a form):
  - Every input has an associated `<label>` or `aria-label` — placeholder is NOT a label
  - Appropriate `autocomplete` hints (`email`, `current-password`, `new-password`, etc.)
  - `<form>` has a real submit handler; Enter-to-submit works
  - Validation errors announced via `aria-live` / `aria-describedby`, not just color
  - Related inputs grouped in `<fieldset>` with `<legend>` where semantic

### 3.5 Motion/Animation Review
- Check reduced-motion support — every non-decorative animation needs a `@media (prefers-reduced-motion: reduce)` alternative
- Verify purposeful animations (enhancement, not payload)
- Check duration/easing consistency
- Flag vestibular-trigger patterns even when reduced-motion is handled: infinite-loop animations on primary content, high-contrast flashes, parallax that moves faster than the scroll
...
```jsonc
{
  ...
  "summary": "[≤3 sentences]",
  "failure_type": "transient|fixable|needs_replan|escalate|null (null when status=completed)",
